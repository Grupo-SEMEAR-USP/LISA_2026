...
class DisplayControlService(Node):
    ...
    def __init__(self):
        ...
        self.callback_group_ = ReentrantCallbackGroup() # grupo que permite rodar o timer e o server em paralelo
    ...
    def display_control_callback(self, request, response):
        self.last_request_time_ = time.time()
        if self.is_sleeping_:   # se estava dormindo, acorda
            self.wake_up()
    ...
    def sleep(self):
        ...
        time.sleep(8) # O gif "sleepy" tem 9s. Spawnamos o gif "sleeping" 1 segundo antes do "sleepy" terminar para evitar da tela piscar

        if not self.is_sleeping_:   # checa se recebeu requisição durante o time.sleep
            self.get_logger().info("Modo soneca cancelado.")
            return

        sleeping_gif_name = "sleeping.gif"
        sleeping_gif_path = os.path.join(self.base_path_,sleeping_gif_name)

        if not os.path.exists(sleeping_gif_path):
            self.get_logger().error(f"Gif {sleeping_gif_path} não encontrado.")
            return

        command = ['mpv', '--fullscreen=yes', '--loop=inf', '--correct-pts=no', '--idle=yes', '--ontop=yes', sleeping_gif_path]
        self.sleeping_gif_process_ = subprocess.Popen(command, env=self.env_, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        self.get_logger().info(f'Gif {sleeping_gif_name} iniciado com sucesso.')
